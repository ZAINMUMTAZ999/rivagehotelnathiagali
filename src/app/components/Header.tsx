...
const Header = () => {
  ...
  // Lock background page scroll while mobile sidebar is open
  useEffect(() => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-blue-900 shadow-sm">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 mr-4">
              <Link href="/dashboard">
                <Button
                  size="sm"
                  variant="outline"
                  className="bg-white active:bg-slate-500 active:scale-95 hover:cursor-pointer text-blue-900 hover:bg-gray-300"
                >
                  Dashboard⚙️
                </Button>
              </Link>
          ...
          >
            {isLogged ? (
              ...
              <Link
                href="/login"
                className="active:bg-slate-500 active:text-slate-700 active:scale-95"
              >
                Login
              </Link>
            )}
        ...
        <button
          className="lg:hidden text-white p-2 hover:bg-blue-800 rounded-md transition-colors active:bg-slate-500 active:text-slate-700 active:scale-95"
      ...
      >
        ...
        {/* Sidebar */}
        <aside
          className={`absolute right-0 top-0 h-full w-80 max-w-[85vw] overflow-y-auto bg-blue-900 shadow-xl transform transition-transform duration-300 ${
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-blue-800">
            ...
            {/* Close Button */}
            <button
              className="text-white p-2 hover:bg-blue-800 rounded-md transition-colors active:bg-slate-500 active:text-slate-700 active:scale-95"
              onClick={closeMobileNav}
          ...
          <nav className="flex flex-col p-4">
            ...
            {isAdmin && (
              <Link href="/dashboard" onClick={closeMobileNav}>
                <Button
                  className="w-full mb-2 text-white bg-blue-600 hover:bg-blue-700 active:bg-slate-500 active:text-slate-700 active:scale-95"
                  onClick={closeMobileNav}
                >
                  Dashboard⚙️
                </Button>
