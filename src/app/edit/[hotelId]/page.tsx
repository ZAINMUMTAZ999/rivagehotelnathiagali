...
export default function EditHotelsById() {
  ...
  const { isAdmin, showToast } = AppContext();

  const queryClient = useQueryClient(); // Initialize query client
  const router = useRouter(); // Initialize router // 1. Fetch Existing Data
  ...
  const isUpdating = updateMutation.isPending; // ⬅️ USED for button disabling // Use useEffect to set default values when the hotel data is fetched

  useEffect(() => {
    if (hotel) {
      reset(hotel);
    }
  }, [hotel, reset]); // Handler for deleting an existing image
...
                </div> */}
                ...
                <div className="p-6 sm:p-8 lg:p-10">
                  ...
                  <div className="flex items-center space-x-3 mb-8">
                    ...
                    <p>( by touching the photo u can delete</p>)                
                     
                  </div>
                                   
                    ...
                    {existingImageUrls.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                        ...
                        {existingImageUrls.map((url) => (
                          <div
                            key={url}
                            className="relative aspect-square overflow-hidden rounded-xl border border-slate-300 shadow-sm"
                          >
                                                       
                            <Image
                              src={url}
                            ...
                            <button
                              ...
                              className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10"
                              type="button"
                            >
                                                           
                              <svg
                        ...
                        <div>
                          ...
                          <label className="cursor-pointer">
                            ...
                            <input
                              ...
                              {...register("imageFiles", {
                                validate: (files) => {
                                  const newFilesCount = files
                                    ? files.length
                                    : 0;
                                  const totalImages =
                                    existingImageUrls.length + newFilesCount;
                                  if (totalImages === 0)
                                    return "Please upload at least one image";
                                  if (totalImages > 6)
                                    return `Maximum 6 images allowed. You have ${existingImageUrls.length} existing images and selected ${newFilesCount} new files.`;
                                  return true;
                                },
                              })}
                            />
                          ...
                          <p className="text-sm text-slate-500 mt-2">
                            Upload up to {6 - existingImageUrls.length} more
                            images (Max 6 total).
                          </p>
                                              
                        </div>
...
                </span> */}
                <span className="flex justify-center items-center mb-3">
                  <button
                    disabled={isLoading || isUpdating}
                    type="submit"
                    className="bg-blue-600 hover:cursor-pointer flex justify-center items-center text-white p-4 font-bold hover:bg-green-500 text-xl disabled:bg-gray-500"
                  >
                    {isLoading
                      ? "Loading..."
                      : isUpdating
